|slug|tenant-and-resource-management|
|---|---|
...
<h4>Note</h4>
<p>For an AI cluster (shared storage), the following fields are added to the resource unit configuration:<ul><li><b>DATA_DISK_SIZE</b>: The size of the tenant's cache disk space. Default value: MAX(min cpu * 2GB, memory size * 2). Minimum value: 2 * cpu (GB).</li><li><b>MAX_NET_BANDWIDTH</b>: The maximum network bandwidth.</li><li><b>NET_BANDWIDTH_WEIGHT</b>: The network bandwidth weight.</li></ul></p>
</main>
