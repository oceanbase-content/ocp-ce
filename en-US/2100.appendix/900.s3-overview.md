...
## Use object storage in shared storage mode

In addition to being used as a backup medium, an object storage service compatible with the S3 protocol can also serve as the primary data persistence medium for an AI cluster in shared storage mode. In shared storage mode, OceanBase Database configures object storage information using the `SHARED_STORAGE_INFO` startup parameter to decouple computing from storage.

The `SHARED_STORAGE_INFO` parameter contains the following main fields:

| Field | Description |
|-------------|---------|
| bucket_uri  | The bucket URI of the object storage service, for example, `s3://oceanbase-test-bucket/data`. |
| host        | The domain name or endpoint of the object storage service. |
| access_id   | The access key ID of the object storage service. |
| access_key  | The access key of the object storage service. |
| max_iops    | The maximum IOPS limit of the object storage service. |
| max_bandwidth | The maximum bandwidth limit of the object storage service. |


For information about how to configure object storage when creating an AI cluster, see [Create an AI cluster](../600.cluster-functions/200.create-a-cluster/300.create-an-ai-cluster.md).
