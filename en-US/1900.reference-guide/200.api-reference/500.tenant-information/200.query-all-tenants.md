...
| mode      | StringArray | No       | ORACLE   | The tenant mode. <li>ORACLE: Oracle mode</li><li>MYSQL: MySQL mode</li> |
...
| ├─ contents      | Array    | A list of tenant information. For more information, see **Tenant data structure**.   |
...
| obClusterId | Integer  | The ID of the cluster to which the tenant belongs on the OceanBase side.    |
...
| syncStatus  | Enum     | The primary-standby synchronization status. Valid values: <li>VALID: valid state</li><li>DISABLED: disabled state, data synchronization is disabled</li><li>DISABLED_WITH_READ_ONLY: read-only state</li><li>SYNCHRONIZING: synchronization in progress (this state does not exist on the OceanBase side)</li><li>UNKNOWN: unknown state</li>  |
| mode        | String   | The tenant mode. Valid values: <li>ORACLE: Oracle mode</li><li>MYSQL: MySQL mode</li>    |
