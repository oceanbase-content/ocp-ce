...
After an alert is generated in OCP, the alert message is sent through a specified alert channel. You can use a shell script or a Python scrip to configure an alert channel. The following example shows how to send an alert message through an alert channel. The `send_alarm` function retrieves the values of alert-related environment variables from the os.environ object.

Here is a Python script example. For more examples, see [Examples of alert channel configuration](../../../860.alert-management/500.manage-alert-channels/600.examples-of-alert-channel-configuration.md).
...
When integrating OCP's alerts into the user's alert platform, the API for querying alert events must be used. For more information, see [Obtain current alerts](200.fetch-actived-alarms.md) or [Obtain alert history](300.fetch-history-alarms.md).
