...
To initiate an offline tenant restore across OCPs from a global view (where the source cluster is not managed by the current OCP), see [Initiate tenant restore globally](2800.global-initiate-tenant-recovery.md).
...
| restoreObj | String | No | Object type to restore. Valid values:<li>tenant: Restore a tenant (default).</li><li>database: Restore a database.</li><li>table: Restore a table.</li> |
