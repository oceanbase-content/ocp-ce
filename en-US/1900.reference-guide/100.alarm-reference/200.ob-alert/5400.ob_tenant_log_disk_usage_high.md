|slug|ob-tenant-log-disk-usage-high-oceanbase-tenant-log-disk-usage-exceeded-the-limit|
|---|---|
...
| Parameter | Value |
| --- | --- |
| Metric | ob_tenant_log_disk_usage, which indicates the log disk usage of a tenant. An alert is triggered when the usage exceeds the threshold. |
| Metric expression | ```100 * avg(ob_tenant_log_disk_used_bytes{@LABELS}) by (@GBLABELS) / avg(ob_tenant_log_disk_total_bytes{@LABELS}) by (@GBLABELS)```   |
| Collected metric | ob_tenant_log_disk_used_bytes, ob_tenant_log_disk_total_bytes |
| Source | Internal views. SQL statements for metric collection: ```SELECT T1.UNIT_ID, T1.SVR_IP, T1.SVR_PORT, T3.TENANT_ID, T3.TENANT_NAME, T1.LOG_DISK_SIZE, T1.LOG_DISK_IN_USE FROM (SELECT UNIT_ID, SVR_IP, SVR_PORT, SUM(LOG_DISK_SIZE) AS LOG_DISK_SIZE, SUM(LOG_DISK_IN_USE) AS LOG_DISK_IN_USE FROM V$OB_UNITS GROUP BY UNIT_ID) T1 JOIN DBA_OB_UNITS T2 JOIN (SELECT TENANT_ID, TENANT_NAME FROM DBA_OB_TENANTS WHERE TENANT_TYPE IN ('SYS', 'USER')) T3 ON T1.UNIT_ID = T2.UNIT_ID AND T2.TENANT_ID = T3.TENANT_ID```   |
| Collection cycle | 5 seconds  |

## Alert rule

| Metric expression | Default threshold (percentage) | Duration | Detection cycle | Time before clearance |
| --- | --- | --- | --- | --- |
| ob_tenant_log_disk_usage | 85   | 0 seconds     | 20 seconds   | 5 minutes   |
