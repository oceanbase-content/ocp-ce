...
<main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>For OCP with MetaDB version 1479, please postpone upgrading to OCP V4.0.1. If an upgrade is necessary, contact technical support.</p>
    </main>
...
In the **<UI-TERM key="ocp-v2.page.Overview.SafetyDiagnosis">Security</UI-TERM>** module, two new modules have been added: **<UI-TERM key="ocp-v2.page.Diagnosis.HighRiskSql">High Risk SQLs</UI-TERM>** and **<UI-TERM key="ocp-v2.page.Overview.AddSql">New SQLs</UI-TERM>**.
...
2. On the tenant's monitoring page, a new diagnostic entry button **<UI-TERM key="ocp-v2.src.util.menu.SqlDiagnosis">SQL Diagnosis</UI-TERM>** has been added. Clicking this jumps you to the SQL diagnosis page. After selecting a time range, click **<UI-TERM key="ocp-v2.page.QueryLog.Query">Search</UI-TERM>** to view SQL executions during that period and locate issues.
...
* Supports customizing the Agent installation directory (parameter: `ocp.agent.home.path.prefix`).
...
1. Fixed the issue where tenant allowlist synchronization failed after upgrading OCP from an older version to V3.3.3 in certain scenarios.

2. Fixed the issue where the unit of measurement for the io_util and io_qusize metrics was incorrect in OCP alerts.

3. Fixed the issue where OCP still executed the upgrade script with the original RS after an RS change occurred in some cases.

4. Fixed the issue where manual and automatic refreshes for the replica view in cluster resource management called different interfaces.

5. Fixed the issue where the server reported the error "Error writing JSON output" when binding Schema indexes for SlowSQL trace details.

6. Fixed the issue where the proxyro@sys password for the Meta cluster stored in the password box was incorrect during Antman deployment of OCP.

7. Fixed the issue where an agent started via SSH could not find the `sysctl` command due to an inherited PATH directory containing too few directories.

8. Fixed the issue where an OBProxy cluster could be permanently locked and inaccessible for maintenance in some cases.

9. Fixed the issue where the upgrade path could not be found when OCP uploaded the debuginfo package for the target version.
