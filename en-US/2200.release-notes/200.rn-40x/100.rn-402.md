...
* **Supported upgrade path:**
    ...
    * For OCP versions earlier than V2.3.0, you must first upgrade to OCP V2.3.x, then to OCP V3.3.4.
    ...
    <h4>Notice</h4>
    <p>For OCP with MetaDB version 1479, some table fields may not be upgradable when upgrading to V4.0.2. If you need to upgrade, it is recommended to contact technical support first.</p>
    </main>
...
* Added built-in alert items:
  ...
  * `ocp_meta_db_disconnected`: Supports detection and alerting for disconnections to OCP's MetaDB or MonitorDB.
...
* Optimized alert recovery strategies:
  ...
  * Threshold-based alerts: For example, alerts like `xxx_count > 1`. An alert will automatically recover only when `xxx_count <=1` is detected for more than its recovery period. If monitoring is lost, the alert will not automatically recover.
...
* Optimized alert detection logic:
  ...
  * Supports users to independently adjust the alert detection cycle. A smaller cycle results in lower alert latency. The theoretical latency is **detection cycle + data collection cycle + 10 s** (the offset for real-time monitoring. To avoid inaccuracies in real-time data, it can be adjusted using the system parameter `ocp.alarm.query.metric-offset-seconds`).
...
1. Fixed an issue where adding an OBServer node to an OceanBase cluster might fail in certain situations.
...
8. Optimized several frontend information display issues that were incomplete.
