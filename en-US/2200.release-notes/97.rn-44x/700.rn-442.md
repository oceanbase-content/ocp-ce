...
Fixed an issue where old resources were not properly released during periodic internal resource refreshes by the OCP monitoring module, causing the OCP process memory to continuously increase over time. This memory leak was more likely to occur in large-scale cluster environments (the more monitoring targets, the more resources consumed per leak, and the faster memory growth), especially when SLS or Prometheus monitoring data sources were enabled, as it could lead to OOM due to exhausted OCP process memory after prolonged operation, affecting control stability. We recommend that users on versions 4.4.1.x/4.4.2.x, particularly those managing large-scale clusters or with SLS/Prometheus monitoring data sources enabled, upgrade as soon as possible. If upgrading is not feasible in the near term, please contact technical support.
...
**Cluster management**: The Transparent Data Encryption (TDE) feature has been adapted to work with Jiangnan Tianan's KMS service, providing users with a more robust data protection solution. Supported OceanBase versions include: [V4.2.5.4, V4.3.0.0), [V4.3.5.2, +∞).
...
* Added three time ranges to the time selector on the **<UI-TERM key="ocp-v2.Detail.Monitor.Title.PerformanceMonitoring">Performance Monitoring</UI-TERM>** page: **<UI-TERM key="ocp-v2.component.OCPRangePicker.constant.NearlyMinutes">Last 5 Minutes</UI-TERM>**, **<UI-TERM key="ocp-v2.Information.Detail.CreateCollectDrawer.NearlyMinutes">Last 10 Minutes</UI-TERM>**, and **<UI-TERM key="ocp-v2.Alarm.Overview.NearlyDays">Last 7 Days</UI-TERM>**. Removed the less frequently used **<UI-TERM key="ocp-v2.component.OCPRangePicker.constant.NearlyMinute">Last 1 Minute</UI-TERM>** option to improve user query efficiency.
