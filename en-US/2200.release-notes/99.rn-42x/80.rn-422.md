...
* Adapts to the replication roles of primary and standby databases in the network for OceanBase Oracle-compatible tenant mode, simplifying user authorization operations.
* Supports GIS types in OceanBase Oracle-compatible tenant mode.
* Supports granting global PL privileges to users in MySQL-compatible mode of OceanBase Database.

#### Monitoring Dashboard

The monitoring dashboard primarily meets user needs such as "monitoring the dashboard" scenarios like during Double 11 and user-defined monitoring views. It provides features like monitoring data comparison for multiple instances and metrics, chart linkage, and supports seven chart display types including pie charts, tables, and bar charts. This helps in monitoring the overall performance and operational status of the database system. For more details, see [Monitoring dashboard](https://www.oceanbase.com/docs/common-ocp-1000000000584639).
...
* **Cluster management**
  * Supports the ability to migrate out an OceanBase cluster, fully realizing closed-loop management capabilities for OceanBase Database. For details, see [Migrate out a cluster](https://www.oceanbase.com/docs/common-ocp-1000000000585002).
  * Supports taking over and migrating out arbitration services, improving closed-loop management capabilities for arbitration services. For details, see [Take over an arbitration service](https://www.oceanbase.com/docs/common-ocp-1000000000585033) and [Migrate out an arbitration service](https://www.oceanbase.com/docs/common-ocp-1000000000585035).
* **OBProxy management**
  * Supports the ability to migrate out an OBProxy cluster, fully realizing closed-loop management capabilities for OBProxy. For details, see [Migrate out an OBProxy cluster](https://www.oceanbase.com/docs/common-ocp-1000000000584804).
  * The log service module now displays routing diagnostics, connection diagnostics, and error logs for the OBProxy service.
* **Host management**
  * When adding a host, it now supports checking whether the system parameter `current_clocksource=tsc` is configured correctly, avoiding performance issues caused by incorrect configurations.
* **Alert center**
  * Added alert capability for OBProxy process exceptions and crashes.
  * Added checks for the installation directory, log disk path, and existence of data files for OBServer nodes, helping users promptly identify abnormal behaviors such as misoperations.
...
* **Performance monitoring**
  ...
  * Added process-level resource usage monitoring for host resources: OBServer/OCP Agent/OBProxy resources such as CPU, memory, and disk I/O.
...
* **<UI-TERM key="ocp-v2.Component.ArbitrationServerList.ArbitrationService">Arbitration Service</UI-TERM>**
  * The service addresses in the cluster arbitration service list support navigating to the arbitration management interface.
* **Cluster/Tenant Management**
  * Added prompts for single-node, single-zone multi-host, and zone configurations that do not meet high data availability requirements, to prevent business continuity failures.
  * Supports quick copying of cluster and tenant names.
* **OBProxy Management**
  * Added status filtering capabilities for the OBProxy list and connectable OceanBase clusters.
...
* **<UI-TERM key="ocp-v2.page.Alarm.Alarm">Alert Center</UI-TERM>**
  ...
  * Added a prompt: For the same alert rule on the same object, the alert rule with a higher priority is applied from the enabled alert templates.
  * Optimized host organization at the OceanBase/OBProxy cluster level, making it easier for users to select objects from a business perspective.
* **<UI-TERM key="ocp-v2.src.hook.useMenu.LogService">Log Services</UI-TERM>**
  * Optimized host organization at the OceanBase/OBProxy cluster level, making it easier for users to select objects from a business perspective.
* **<UI-TERM key="ocp-v2.Detail.Realtime.PerformanceMonitoring">Performance Monitoring</UI-TERM>**
...
*  **Tenant Management**
    * Documentation change: End-to-end diagnostics has been changed to end-to-end tracing.
*  **Backup and Recovery**
    * Documentation change: The restore time in the sample inspection task list has been changed to restore time point.

### Key Feature Interpretation

#### Monitoring Dashboard

The monitoring dashboard primarily meets user needs such as "monitoring the dashboard" during events like Double 11 and the requirement for custom monitoring views. On the Monitoring Dashboard tab, you can create multiple monitoring dashboard pages. On each page, you can add various charts to display different metrics and data, such as multi-instance user monitoring, associated viewing of multi-metric dashboards, and different monitoring metrics for one or multiple clusters. Additionally, the monitoring dashboard provides four comparison methods: year-over-year, day-over-day, week-over-week, and month-over-month, helping you easily identify data anomalies. You can also choose from seven different chart types based on the characteristics and requirements of your data, such as line charts, pie charts, and tables, which helps you better understand and analyze the data. Furthermore, the monitoring dashboard supports grouping, allowing you to place related metrics into the same group for unified management, thereby facilitating better monitoring and analysis of database conditions. The monitoring dashboard supports features like chart dragging, zooming, drilling down (in single-metric scenarios), and chart linkage, helping you achieve the goal of monitoring the overall performance and operational status of the database system.

  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>A single monitoring dashboard supports up to 32 charts.</p>
  </main>

#### Tag Management

OCP supports tagging five object types: OceanBase clusters, arbitration services, tenants, OBProxy clusters, and hosts. It also supports management functions such as creating, editing, deleting, and modifying associated objects. With two different styles, you can choose the appropriate tag display format based on your business situation. On the interfaces for various object types, such as an OceanBase cluster, you can perform tagging operations to assign business meanings to objects, such as system type or manager. Additionally, the tag filtering capability helps you organize, search, and manage cluster objects, improving your work efficiency.
...
* Fixed the issue where starting `obproxyd.sh` failed after a server restart.
...
* Improved access performance on multiple frontend pages, making access faster under the same management scale.
