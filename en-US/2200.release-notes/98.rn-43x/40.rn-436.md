...
* **Agent backup timeout** (`backup.dump.physical.file.timeout`): controls the timeout for physical backup file dumping.
* **S3 storage path style** (`backup.storage.s3.use.path.style`): configures the path access method for S3 buckets.
...
|-------|---------|---------------|
| 1 | When the value of the `backup.storage.s3.use.path.style` parameter is `false`, S3 does not support using COS storage paths. | Change the value of the `backup.storage.s3.use.path.style` parameter to `true`. |
