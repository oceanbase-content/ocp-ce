...
除了作为备份介质外，兼容 S3 协议的对象存储还可作为 AI 集群（共享存储形态）的主数据持久化介质。在共享存储形态下，OceanBase 数据库通过 `SHARED_STORAGE_INFO` 启动参数配置对象存储信息，实现计算与存储的解耦。

`SHARED_STORAGE_INFO` 参数包含以下主要字段：

|     字段     |     说明   |
|-------------|---------|
| bucket_uri  | 对象存储的 Bucket URI，例如 `s3://oceanbase-test-bucket/data`。 |
| host        | 对象存储服务的域名或 Endpoint。 |
| access_id   | 对象存储的访问密钥 ID。 |
| access_key  | 对象存储的访问密钥。 |
| ram_url     | RAM 角色 URL，用于通过 RAM 角色方式访问对象存储（可选）。 |
| max_iops    | 对象存储的最大 IOPS 限制。 |
| max_bandwidth | 对象存储的最大带宽限制。 |
| used_for_type | 对象存储的用途类型。 |

关于如何在创建 AI 集群时配置对象存储，详情参见 [创建 AI 集群](../600.cluster-functions/200.create-a-cluster/300.create-an-ai-cluster.md)。
