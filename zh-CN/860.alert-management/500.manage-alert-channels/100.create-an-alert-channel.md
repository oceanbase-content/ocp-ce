...
根据界面提示设置通道的基本信息。不同通道类型，所需填写的基本信息会有不同。

当 **<UI-TERM key="ocp-v2.Alarm.Channel.New.ChannelType">通道类型</UI-TERM>** 选择为 **HTTP**、**<UI-TERM key="ocp-v2.Auth.User.New.Mailbox">邮箱</UI-TERM>** 或 **<UI-TERM key="ocp-v2.src.constant.alarm.CustomScript">自定义脚本</UI-TERM>** 时，需根据界面提示进行通道配置。以 **HTTP** 通道类型为例，具体配置说明如下：
...
2. 根据实际所需，对告警消息进行个性化配置。
   ...
   3. 选择是否开启 **<UI-TERM key="ocp-v2.Alarm.Channel.Detail.AlertMessageAggregation">告警消息聚合</UI-TERM>** 功能。开启消息聚合，可以避免一段时间内告警发送过多，导致重要消息被淹没。开启告警消息聚合后，系统将依照您所配置的模板样式及间隔发送告警消息。

3. 根据实际所需，对恢复消息进行个性化配置。

   1. 配置告警 **消息模板**，系统所推送的消息将依照您所配置的模板样式进行展示。

   2. 选择是否开启 **<UI-TERM key="ocp-v2.Alarm.Channel.Detail.AlertMessageAggregation">恢复消息聚合</UI-TERM>** 功能。开启消息聚合，可以避免一段时间内恢复消息发送过多，导致重要消息被淹没。开启恢复消息聚合后，系统将依照您所配置的模板样式及间隔发送恢复消息。

### 步骤四：发送测试消息

单击 **<UI-TERM key="ocp-v2.Alarm.Channel.New.SendTestMessages">发送测试消息</UI-TERM>** ，测试当前配置的告警通道是否可以成功发送告警消息。

### 步骤五：提交告警通道配置

若测试消息发送成功，可单击 **<UI-TERM key="ocp-v2.Inspection.Item.ConfigRule.Submit">提交</UI-TERM>**，保存此告警通道配置。
