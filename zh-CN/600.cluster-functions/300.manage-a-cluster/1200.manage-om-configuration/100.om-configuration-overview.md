...
OCP 支持的采集和运维参数共分为五类，包括：**<UI-TERM key="ocp-v2.component.OperationConfig.List.SqlDiagnosticCollection">SQL 诊断采集</UI-TERM>**、**<UI-TERM key="ocp-v2.src.constant.parameter.TransactionDiagnosticCollection">事务诊断采集</UI-TERM>**、**<UI-TERM key="ocp-v2.src.constant.parameter.ActiveSessionHistoryCollection">活跃会话历史采集</UI-TERM>**、**<UI-TERM key="ocp-v2.src.constant.parameter.PerformanceMonitoringParameters">性能监控参数</UI-TERM>** 和 **备份参数**，分别对应 OCP 的 SQL 诊断功能、事务诊断功能、活跃会话历史功能、性能监控功能和备份功能。

### SQL 诊断采集

**<UI-TERM key="ocp-v2.src.constant.parameter.SqlDiagnosticCollection">SQL 诊断采集</UI-TERM>** 主要为租户和诊断服务内的 SQL 诊断模块提供基础数据。
...
**<UI-TERM key="ocp-v2.component.OperationConfig.List.TransactionDiagnosticCollection">事务诊断采集</UI-TERM>** 主要为租户和诊断服务内的事务诊断模块提供基础数据。
...
**<UI-TERM key="ocp-v2.src.constant.parameter.ActiveSessionHistoryCollection">活跃会话历史采集</UI-TERM>** 主要为租户和诊断服务内的会话管理模块提供基础数据。
...
**性能监控采集** 主要为集群、租户和 OBProxy 的性能监控模块提供基础数据。
...
**备份采集** 主要为集群和租户的备份模块提供基础数据。

| 参数名 | 参数类型 | 默认值 | 取值范围 | 参数说明 |
|--------|----------|--------|--------|-----------|
| backup.dump.physical.file.timeout | Int    | 30 | [10, 300] | 物理备份文件转储超时时间，单位为秒。 |
| backup.storage.s3.use.path.style  | Bool   | true | - | S3 存储是否使用路径样式访问。 |
