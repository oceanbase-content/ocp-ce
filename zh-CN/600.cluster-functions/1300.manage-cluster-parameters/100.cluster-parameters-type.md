...
当集群为 AI 集群（共享存储形态，`ob_startup_mode=shared_storage`）时，部分集群参数的语义发生变化，具体如下：

|     参数     |     说明   |
|-------------|---------|
| datafile_size | 在共享存储形态下，该参数表示缓存空间大小，而非数据文件大小。 |
| datafile_disk_percentage | 在共享存储形态下，该参数表示缓存空间百分比。 |
| datafile_next | 在共享存储形态下不支持该参数。 |
| datafile_maxsize | 在共享存储形态下不支持该参数。 |
| _datafile_usage_upper_bound_percentage | 在共享存储形态下不支持该参数。 |
| _datafile_usage_lower_bound_percentage | 在共享存储形态下不支持该参数。 |

此外，共享存储形态下新增以下租户级参数：

|     参数     |     说明   |
|-------------|---------|
| shared_log_retention | 共享日志保留时间，默认值为 1d，取值范围为 [0d, 14d]。 |
| read_cache_disk_percentage | 读缓存盘百分比，默认值为 40，取值范围为 [0, 90]。 |
| cache_disk_memory_limit | 缓存盘内存限制，默认值为 10，取值范围为 [5, 50]。 |
