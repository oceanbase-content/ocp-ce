...
OceanBase 数据库目前支持使用 File（NFS）、OSS、OBS 和 S3 四种存储介质作为备份的目的地。以下两个图是备份功能在备份目的地创建的目录结构以及每个目录下保存的文件类型，其中图一适用于 V4.x 版本的 OceanBase 集群，图二适用于 V3.x 版本的 OceanBase 集群。
