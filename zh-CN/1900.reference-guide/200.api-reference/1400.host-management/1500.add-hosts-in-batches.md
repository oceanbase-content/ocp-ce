...
|---|---|
|machine-translation||
|slug|add-hosts-in-batches-of-api|
...
| vpcId             | Long        | 否  | 100                        | VPC ID。   |
| typeId            | Long        | 是  | 1001                       | 机型 ID。   |
| credentialId      | Long        | 是  | 1001                       | 凭据 ID，凭据是 SSH 用来访问主机的信息。   |
| alias             | String      | 否  | 机器 1                     | 主机的别名，只有添加单台时才可以设置别名。  |
| description       | String      | 否  | 测试机器                   | 主机描述信息。  |
| mgragentPort      | Integer     | 否  | 62889                      | OCP-Agent 监控数据监听端口，默认为 62889。 |
| monagentPort      | Integer     | 否  | 62888                      | OCP-Agent API 监听端口，默认为 62888。 |
| monagentCpuQuota  | number      | 否  | 2.0                        | ocp_monagent 的 CPU 配置，需大于等于 0.5C，请按 0.5C 倍数递增，建议最大值为 6C。 |
| monagentMemoryQuota | Integer    | 否  | 2                         | ocp_monagent 的内存配置，需大于等于 1GiB，请按 1GiB 倍数递增，建议最大值为 12GiB。 |
