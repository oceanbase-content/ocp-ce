...
|---|---|
|machine-translation||
|slug|create-a-standby-tenant-of-api|
...
| standbyROPassword | String      | 否 | tenantA | 备租户用于日志同步的只读用户密码。 |
...
| serviceName       | String      | 否 | Service_A | 服务名。 |
...
| 参数              | 类型    | 必选   | 示例值   | 描述  |
|-------------------|---------|-------|----------|-------|
| useCustomStorage  | boolean | 否    | true     | 是否使用自定义的存储目录，若该值为 true，dataBackupUrl 和 logBackupUrl 不能是空。  |
...
| storageUrl        | String  | 是    | 备份策略中的存储路径URL。请配置为不带协议的目录，如 `/obbackup/` 或 `osstest/backup`。    |
