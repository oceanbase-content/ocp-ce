...
2. 为这些 SQL 增加并发执行限制的 Hint。
   ...
   2. 为这些 SQL 文本增加 HINT 索引 **`/*+max_concurrent(1)*/`** **，** 该索引将限制这个 SQL 文本的并发数为 1，从而限制该 SQL 的执行速度。可参考 [绑定索引](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000001053018) 或下述语句进行限流。
