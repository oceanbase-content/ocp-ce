...
OCP 支持部署在不同时区环境，但同一个 OCP 下所有节点的时区必须保持统一。在 OCP 运行过程中，禁止修改 MetaDB 的时区。管控层面，OCP 支持管控不同时区的 OceanBase 集群、OceanBase 租户以及跨时区的主备集群和主备租户等，但需要特别注意，由于 OceanBase 内部表与视图时区转换逻辑的限制，必须保证租户的 `system_time_zone` 和 `time_zone` 参数值一致，否则 OCP 功能可能出现问题。
