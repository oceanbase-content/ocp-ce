...
|   自定义 Zone |   开启自定义 Zone，需要填写 Zone 名称、机房（IDC）、服务器、Root Server 位置。 <main id="notice" type='explain'><h4>说明</h4>此参数与 <b>服务器</b> 参数二选一，当打开自定义 Zone 后，服务器参数会自动隐藏。</main>    |
|   配置仲裁服务  | 可自由开启或者关闭，开启时可以选择已安装的仲裁服务。如果没有提前创建仲裁服务或者不选择已安装的仲裁服务，也可以开启新建仲裁服务按钮，填写相关信息新建仲裁服务。<main id="notice" type='explain'><h4>说明</h4><ul><li>当 <b>MetaDB 镜像</b> 选择 V4.x 版本的 MetaDB 时，才能配置仲裁服务。</li><li>选择已创建仲裁服务时，只支持选择和 MetaDB 镜像同架构的仲裁服务。</li><li>新建仲裁服务时，仲裁服务镜像要和 MetaDB 镜像同架构，服务器用途为仲裁服务或者 OceanBase 产品服务。</li></ul></main>    |
...
|   内存   |   MetaDB 在服务器上可用的内存，默认为 90GiB 。   |
...
<h4>说明</h4>
<ul>
<li>MetaDB 所需的 CPU 和内存资源为 <a href="../100.planning-resources/200.metadb-resources.md">MetaDB 资源</a>、<a href="../100.planning-resources/300.monitordb-resources.md">MonitorDB 资源</a>、sys 租户、system_memory 和 OBProxy 等资源的累加值，在为 MetaDB 分配相关资源时，建议您除所需资源外预留一定的余量（系统租户本身也会占用一定资源）。</li>
...
1. MetaDB 配置完成后，单击 **<UI-TERM key="ocp-v2.Settings.Component.ModifyUserInfoDrawer.Submit">提交</UI-TERM>**。如果您的配置有误，可单击 **<UI-TERM key="ocp-v2.Alarm.Subscriptions.Reset">重置</UI-TERM>** 一键恢复默认配置。

2. 提交创建 MetaDB 成功后，进入 **系统与安全** > **<UI-TERM key="ocp-v2.Layout.BasicLayout.TaskCenter">任务中心</UI-TERM>**，单击任务名称，可查看详细任务信息。
